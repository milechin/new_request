...
Creates `${DIR}/${CLIENT}/${TICKET}/` (DIR defaults to `pwd`), populates it with `data/`, `env_setup/`, `scripts/`, `output/`, `context/`, a `.gitignore`, a `module_load.sh` (self-documenting header + home-isolation block), a per-workspace `CLAUDE.md` + `context/` templates (copied from `templates/`), a symlinked `.claude/` (→ this repo's `.claude/`, so all of its commands/skills/agents are available — currently the `/init-request` slash command), and runs `git init` inside it. The R helpers are **not** copied in — they live centrally in this repo's `bin/` (`r_env.sh`, `r_snapshot.sh`) and are run against a workspace (see below). The script exits with status 1 (and prints `Help`) if CLIENT or TICKET is missing, contains `/` or `..`, or if the target directory already exists; `-h`/`--help` prints help and exits 0.
...
- **`.claude/`** — a **symlink** to this repo's own `.claude/` (so every command/skill/agent in the repo is available in every workspace, and adding a new one needs no re-scaffolding). The canonical assets live at `.claude/commands/` (e.g. `init-request.md`), `.claude/skills/`, etc. in this repo. The `/init-request` command reads `CLAUDE.md` + `context/` + `scripts/` + the environment and writes `context/SUMMARY.md`. The workspace `.claude` symlink is gitignored (it's an absolute path into this repo — don't commit it; the `.gitignore` entry is `.claude`, no trailing slash, so it matches the symlink); Claude Code still discovers everything through it.
